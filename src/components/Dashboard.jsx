...
export const Dashboard = ({ 
  ...
  return (
    <header className="flex flex-col tablet:flex-row gap-2 tablet:gap-[4px] mb-8 w-full">
      {/* BLOQUE IZQUIERDO: TITULO */}
      <div className="w-full tablet:flex-1 bg-mines-blue-light border border-mines-grey-light rounded-md p-6 flex flex-col items-center justify-center tablet:h-[200px]">
      ...
      {/* BLOQUE CENTRAL: INFO O RESULTADO - FORZADO CON min-h-40 EN MOVIL */}
      <div className="w-full tablet:flex-1 bg-mines-blue-light border border-mines-grey-light rounded-md min-h-40 tablet:h-[200px] relative overflow-hidden text-center">
        ...
        <div className={`absolute inset-0 flex flex-col items-center justify-center transition-all duration-500 ${!isPlaying ? 'opacity-100' : 'opacity-0 pointer-events-none -translate-y-4'}`}>
          <h3 className={`text-3xl font-bold uppercase tracking-tighter mb-4 ${
            gameState === 'won' ? 'text-green-800' : 'text-red-600'
          }`}>
            {gameState === 'won' ? '¡Enhorabuena!' : '¡Has fallado!'}
          </h3>
          <button 
            onClick={onNewGame}
            className="bg-white border border-slate-300 px-6 py-2 rounded text-slate-800 font-bold hover:bg-slate-50 transition-all shadow-sm cursor-pointer uppercase text-sm"
          >
            Nueva Partida
          </button>
        </div>
      ...
      {/* BLOQUE DERECHO: RECORDS */}
      <div className="w-full tablet:flex-1 bg-mines-blue-light border border-mines-grey-light rounded-md p-6 flex flex-col justify-center tablet:h-[200px]">
        <p className="text-xl font-bold mb-4 flex items-center justify-center gap-4 text-slate-700 uppercase">
          Récords 
          <img 
            src={resetIcon} 
            onClick={onResetRecords} 
            className="w-6 h-6 cursor-pointer hover:rotate-180 transition-transform duration-500" 
            alt="reset" 
          />
        </p>
